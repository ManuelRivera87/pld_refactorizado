...
export type SalesReportValidationError = {
  rowNumber: number;
  field: string;
  column: string;
  message: string;
...
const rules = {
  ...
  digito1: createRule("\\d{1}", 1, 1, "1 digito"),
  ...
  digito4: createRule("\\d{4}", 4, 4, "4 digitos"),
  ...
  vehiculo40: createRule(`[${textLetters}\\d \\.,:/]{1,40}`, 1, 40, "descripcion vehicular de 1 a 40 caracteres"),
  vin: createRule("[A-Z0-9\\-_]{17}", 17, 17, "VIN de 17 caracteres"),
  repuve: createRule("[A-Z0-9\\-_]{8}", 8, 8, "REPUVE de 8 caracteres"),
  placas: createRule("[A-Z\\d\\-_]{1,12}", 1, 12, "placas de 1 a 12 caracteres")
};

const columnHeaderByColumn = new Map(
  salesReportColumns.map((column) => [column.column, column.header])
);
...
export const validateSalesReportRows = (
  ...
  const errors: SalesReportValidationError[] = [];
