...
import { validateLeaseReportRows } from "./leaseReportValidation.js";

type UploadLeaseReportInput = {
...
const tipoActividad = "ARI" as const;
...
const insertColumns = [
  ...metadataColumns,
  ...leaseReportColumns.map((column) => column.column)
];
...
const readRows = (buffer: Buffer) => {
  ...
  const firstSheetName = workbook.SheetNames.includes("Informe")
    ? "Informe"
...
const buildHeaderIndex = (headerRow: unknown[]) => {
  ...
  const missingHeaders = leaseReportColumns
...
const hasMappedLeaseValue = (row: unknown[], headerIndex: Map<string, number>) =>
  leaseReportColumns.some((column) => {
...
export const uploadLeaseReport = async ({
  anioAfectacion,
  companyId,
  file,
  mesAfectacion,
  uploadedByUserId
}: UploadLeaseReportInput): Promise<CreditReportUploadSummary> => {
  if (!file) {
  ...
  const dataRows = rows.slice(1).filter((row) => hasMappedLeaseValue(row, headerIndex));
  const validationErrors = validateLeaseReportRows(dataRows, headerIndex);
  ...
  const query = `
    INSERT INTO informe_arrendamiento_registros (${insertColumns.join(", ")})
    VALUES (${placeholders})
  `;
  ...
  try {
    ...
    const uploadResult = await client.query<{ id: string }>(
      ...
      [
        ...
        tipoActividad,
        dataRows.length,
        leaseReportColumns.length + affectationFields.length
      ]
    );
    ...
    return {
      ...
      fieldsInserted: leaseReportColumns.length + affectationFields.length,
      insertedFields: [
        ...affectationFields,
        ...leaseReportColumns.map(({ column, header }) => ({
          column,
          header
        }))
      ]
