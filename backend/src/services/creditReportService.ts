...
import { validateCreditReportRows } from "./creditReportValidation.js";

type UploadCreditReportInput = {
...
const tipoActividad = "MPC" as const;
...
const insertColumns = [
  ...metadataColumns,
  ...creditReportColumns.map((column) => column.column)
];
...
const buildHeaderIndex = (headerRow: unknown[]) => {
  ...
  const missingHeaders = creditReportColumns
...
export const uploadCreditReport = async ({
  anioAfectacion,
  companyId,
  file,
  mesAfectacion,
  uploadedByUserId
}: UploadCreditReportInput): Promise<CreditReportUploadSummary> => {
  if (!file) {
  ...
  const validationErrors = validateCreditReportRows(dataRows, headerIndex);
  ...
  const query = `
    INSERT INTO informe_credito_registros (${insertColumns.join(", ")})
    VALUES (${placeholders})
  `;
  ...
  try {
    ...
    const uploadResult = await client.query<{ id: string }>(
      ...
      [
        ...
        tipoActividad,
        dataRows.length,
        creditReportColumns.length + affectationFields.length
      ]
    );
    ...
    for (const row of dataRows) {
      const values = [
        ...
        parsedAnioAfectacion,
        tipoActividad,
        ...creditReportColumns.map((column) =>
    ...
    return {
      ...
      fieldsInserted: creditReportColumns.length + affectationFields.length,
      insertedFields: [
        ...affectationFields,
        ...creditReportColumns.map(({ column, header }) => ({
          column,
          header
        }))
      ]
